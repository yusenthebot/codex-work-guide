...
Zero `FAIL` items are required. Review every warning, especially component approximation, SVG family mismatch, aspect distortion, image-frame dimension mismatch, route penetration through thin dividers or rails, text overflow, arrow-box collision, overlap, spacing variance, palette scatter, orphan labels, and edge-density hotspots. Read `references/component-and-geometry-audit.md` and `references/xml-preflight.md` when a warning is ambiguous. The screenshot pass must separately confirm optical stroke weight and icon centering because equal numeric frames alone cannot prove optical balance.
