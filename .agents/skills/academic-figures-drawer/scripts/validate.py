...
def check_page(diagram):
    ...
    for i in range(len(boxes)):
        for j in range(i + 1, len(boxes)):
            ...
            # Compound editable icons legitimately use overlapping sibling
            # primitives inside a non-root group (for example links meeting
            # robot joints or a seal over a certificate sheet). Audit the
            # group's outer geometry/containment separately; only treat
            # top-level sibling overlap as a structural warning here.
            if pa == pb and pa in (None, "", "0", "1") and overlap(ra, rb):
